...
Communication::Communication(int rx_pin, int tx_pin) {
  m_rx = rx_pin;
  m_tx = tx_pin;
  m_bufferMaxSize = 128;
}
...
bool Communication::connect() const {
...
void Communication::wakeup() const {
...
bool Communication::initialize() const {
...
CommandResult* Communication::readData() const {
...
void Communication::writeData(uint8_t const* data, size_t len) const {
  Serial2.flush(false);
