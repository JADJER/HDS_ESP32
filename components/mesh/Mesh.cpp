...
static uint8_t net_buf_data_sensor_setup_pub[20];
static struct net_buf_simple bt_mesh_pub_msg_sensor_setup_pub = {
    .data  = net_buf_data_sensor_setup_pub,
    .len   = 0,
    .size  = 20,
    .__buf = net_buf_data_sensor_setup_pub,
};

static esp_ble_mesh_model_pub_t sensor_setup_pub = {
    .msg = &bt_mesh_pub_msg_sensor_setup_pub,
    .update = (uint32_t)nullptr,
    .dev_role = ROLE_NODE,
};
...
static esp_ble_mesh_elem_t elements[] = {{
    ...
    .vnd_model_count = ARRAY_SIZE(ESP_BLE_MESH_MODEL_NONE),
    .sig_models = (root_models),
    .vnd_models = (ESP_BLE_MESH_MODEL_NONE),
}};
...
static void example_ble_mesh_sensor_server_cb(esp_ble_mesh_sensor_server_cb_event_t event, esp_ble_mesh_sensor_server_cb_param_t* param) {
  ESP_LOGI(TAG, "Sensor server, event %d, src 0x%04x, dst 0x%04x, model_id 0x%04x", event, param->ctx.addr, param->ctx.recv_dst, param->model->model_id);
...
esp_err_t Mesh::init() {
  ...
  err = esp_ble_mesh_register_prov_callback(example_ble_mesh_provisioning_cb);
  if (err != ESP_OK) {
